...
This crate depends on the `blam-tags` engine crate **by path**, expecting it as
a sibling checkout:

```
GitHub/
├── genesis/           <- this repo
└── blam-tags/         <- the engine repo (clone it next to this one)
    └── blam-tags/     <- the library crate (referenced as ../blam-tags/blam-tags)
```
